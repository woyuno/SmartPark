...
function UserForm({ visible, hideModal, title, loadData }: FormProps) {
  ...
  useEffect(() => {
    title === '新增企业' ? form.resetFields() : form.setFieldsValue(userDataStore)
  }, [visible])
  return (
    <Modal title={title} open={visible} onCancel={hideModal} width={800} onOk={handleOk}>
      <Form form={form} labelCol={{ span: 8 }} wrapperCol={{ span: 16 }}>
        ...
        <Row gutter={16}>
          ...
          <Col span={12}>
            <Form.Item label="组织机构代码" name="organizationCode" rules={[{ required: true, message: '组织机构代码不能为空' }]}>
              <Input />
            </Form.Item>
          </Col>
        </Row>
